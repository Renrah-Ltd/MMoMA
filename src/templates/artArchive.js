...
const ArtArchive = ({ data, pageContext }) => {
    ...
    const artMap = art.map((art)=> { return (<div className={"art-card " + (art?.forSale && " for-sale")}><Link to={`/art/${art.slug}`}><div class="art-card__image"><img src={art.image} /></div><h2>{art.name}</h2></Link></div>)})
